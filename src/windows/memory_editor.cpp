...
void RenderMemoryEditor(VMState& vm_state) {
    ...
    if (mem_edit.Open) {
        ...
        if (openDialog) {
            FileDialog::ShowFileDialog_s(&openDialog, selectedPath);
        }
        ...
        if (strlen(selectedPath) > 0) {
            ...
            if (f) {
                ...
                // Limpa caminho para não carregar novamente no próximo frame
                selectedPath[0] = '\0';
            } else {
                std::cout << "Erro ao abrir o arquivo: " << selectedPath
                          << "\n";

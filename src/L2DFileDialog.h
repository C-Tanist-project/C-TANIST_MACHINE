  ...
  if (open) {
    ...
    if (!initial_path_set && strlen(buffer) > 0) {
      ...
      if (std::filesystem::is_directory(path)) {
        ...
        // Check if this is just a file in a real path. If so, use the real
        // path. If that still doesn't work, use current path.
    ...
    strcpy_s(pathInput, selected_file_path.c_str());
    ...
    if (ImGui::InputText("##text", pathInput, sizeof(pathInput),
      ...
      if (std::filesystem::exists(newPath)) {
        ...
        strcpy_s(file_dialog_error, "Path inválido.");
      }
    }

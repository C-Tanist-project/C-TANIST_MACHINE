...
inline void ShowFileDialog(bool* open, char* buffer,
    ...
    if (open) {
        // Check if there was already something in the buffer. If so, try to use
        // that path (if it exists). If it doesn't exist, just put them into the
        // current path.
        if (!initial_path_set && strlen(buffer) > 0) {
            ...
            if (std::filesystem::is_directory(path)) {
                ...
                // Check if this is just a file in a real path. If so, use the
                // real path. If that still doesn't work, use current path.
        ...
        strcpy(pathInput, selected_file_path.c_str());

        ImGui::PushItemWidth(724);
        if (ImGui::InputText("##text", pathInput, sizeof(pathInput),
                             ImGuiInputTextFlags_EnterReturnsTrue)) {
            std::filesystem::path newPath = std::filesystem::path(pathInput);
            if (std::filesystem::exists(newPath)) {
                if (std::filesystem::is_directory(newPath)) {
                    file_dialog_current_path = newPath.string();
                    file_dialog_folder_select_index = 0;
                    file_dialog_file_select_index = 0;
                } else {
                    file_dialog_current_path = newPath.parent_path().string();
                    file_dialog_current_file = newPath.filename().string();
                }
            } else {
                strcpy(file_dialog_error, "Path inválido.");
            }
        }
...
inline void ShowFileDialog_s(bool* open, char* buffer,
                             FileDialogType type = FileDialogType::OpenFile) {
